...
X_train = np.array([tweet_num[:-1],  follower_num[:-1],mention_num[:-1], author_num[:-1], ranking_scores[:-1] , reply_num[:-1], favorite_count[:-1], impression[:-1], np.ones(len(tweet_num)-1)])
X_train = X_train.T
...
fig, bx = plt.subplots()
bx.scatter(X_train[:,3], result,color='r', s=60, alpha=.4)
plt.xlabel('Number of authors')
plt.ylabel('Predictant')
plt.show()

fig, cx = plt.subplots()
cx.scatter(X_train[:,4], result,color='b', s=60, alpha=.4)
