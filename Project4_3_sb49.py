...
f = open('tweets_#sb49.txt', 'r', encoding = 'utf-8')
...
X_train = np.array([tweet_num[:-1], retweet_num[:-1], follower_num[:-1], max_follower_num[:-1], mention_num[:-1], author_num[:-1], Cooccurrence_times[:-1] ,ranking_scores[:-1] , reply_num[:-1], favorite_count[:-1], impression[:-1], np.ones(len(tweet_num)-1)])
X_train = X_train.T
...
fig, bx = plt.subplots()
bx.scatter(X_train[:,4], result,color='r', s=60, alpha=.4)
...
fig, cx = plt.subplots()
cx.scatter(X_train[:,7], result,color='b', s=60, alpha=.4)
