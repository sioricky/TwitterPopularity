...
f = open('tweets_#patriots.txt', 'r', encoding = 'utf-8')
...
X_train = np.array([tweet_num[:-1], retweet_num[:-1], follower_num[:-1], max_follower_num[:-1],  mention_num[:-1], author_num[:-1], Cooccurrence_times[:-1] ,ranking_scores[:-1] , reply_num[:-1],  impression[:-1], np.ones(len(tweet_num)-1)])
X_train = X_train.T
