...
#            time_of_the_day[i] = 24
    ...
    X_train = np.array([  author_num[:-1], Cooccurrence_times[:-1] , favorite_count[:-1], np.ones(len(tweet_num)-1)])
    X_train = X_train.T
...
f = open('sample2_period2.txt', 'r', encoding = 'utf-8')
line = f.readline()
tweets = []
while len(line)!=0:
    tweet = json.loads(line)
...
favorite_count=np.zeros(6)
uniqueAuthors_temp=np.zeros(6).tolist()
author_num=np.zeros(6)
for i in range(6):
    uniqueAuthors_temp[i] = []
for i in range(len(tweets)):
    ...
    for j in range(1,7):
        if time==j:            
            favorite_count[j-1] += tweets[i]['tweet']['favorite_count']
            tweet_num[j-1] += 1
#            ranking_scores[j-1] += 1
            if len(tweets[i]['tweet']['entities']['hashtags'])>=2:
                Cooccurrence_times[j-1] += 1  
            if tweets[i]['author']['name'] in uniqueAuthors_temp[j-1]:   
                continue
            else:
                uniqueAuthors_temp[j-1].append(tweets[i]['author']['name'])
                author_num[j-1] += 1
X_test=np.array([author_num[:],Cooccurrence_times[:],favorite_count[:],np.ones(6)])
X_test=X_test.T
